...
export const normalizeDbTimeEntry = entry => {
...
export const serializeIntervalsForStorage = intervals =>
  Array.isArray(intervals) ? intervals.map(serializeInterval) : [];

export const serializeTimeEntryForClient = entry => {
...
export const ensureTimeEntryArrays = entry => {
  const normalized = normalizeDbTimeEntry(entry);
  if (!normalized) {
    return {
      breaks: [],
...
export const toDateOrNullSafe = toDateOrNull;
